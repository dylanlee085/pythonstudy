...
def sendmail(sender,receiver,content):
    ...
    msg.attach(contents)
    filename = '每日入驻数据(2018-03-18).xls'
    xlsxpart = MIMEText(open(filename, 'rb').read(), 'base64', 'gb2312')
    xlsxpart["Content-Type"] = 'application/octet-stream'      #打开附件时类型
    xlsxpart.add_header('Content-Disposition', 'attachment', filename=filename.decode('utf-8').encode('gb2312'))     #使用中文文件名时需要转码
    msg.attach(xlsxpart)
    try:
        server = smtplib.SMTP()
        server.connect(host)
